...
class Registry {
    ...
    /**
     * Create pair of clients waiting for each other's external address
     * @param {string} connectionName
    ...
    createPair(connectionName, serverId, clientId) {
        let clientRequestId = uuid.v1();
        let serverRequestId = uuid.v1();
    ...
    _checkTimeout() {
        let now = Date.now();
        for (let info of this.pairs.values()) {
            if (now >= info.timeout) {
